...
if [[ ! -f /var/opt/mssql/.db-initialized ]] || [[ "$db_exists" == "0" ]]; then
  ...
  if [[ -f /init/init.sql ]]; then
    "$SQLCMD" -b -C -S localhost -U sa -P "$SA_PASSWORD" -d master -v sa_password="$SA_PASSWORD" -i /init/init.sql
  fi
  ...
  if [[ -f /init/novanorth-prod.sql ]]; then
    ...
    for i in {1..20}; do
      ...
      script_output=$("$SQLCMD" -C -S localhost -U sa -P "$SA_PASSWORD" -d master -v sa_password="$SA_PASSWORD" -i /init/novanorth-prod.sql 2>&1 || true)
      printf '%s\n' "$script_output"

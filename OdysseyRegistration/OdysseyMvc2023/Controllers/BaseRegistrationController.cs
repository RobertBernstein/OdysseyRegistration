...
namespace OdysseyMvc2023.Controllers
{
    ...
    public class BaseRegistrationController : Controller
    {
        ...
        private string DetermineSiteCssFile() =>
            this.Request.Url != (Uri)null &&
            this.Request.Url.AbsoluteUri.ToLowerInvariant().Contains("novasouth")
                ? this.Url.Content("~/Content/NovaSouth.css")
                : this.Url.Content("~/Content/NovaNorth.css");

        private string DetermineSiteName()
        {
            string siteName = this.Request.Url != (Uri)null
